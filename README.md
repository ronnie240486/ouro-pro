...
A sincronização de séries foi protegida contra perda de dados. Respostas vazias de `get_series`, `get_second_series` ou episódios não apagam mais registros do Realm nem substituem as categorias por uma lista vazia. Quando a tela encontra menos de 100 séries, ela compara `get_series` e `get_second_series` e usa a maior resposta válida em segundo plano. Consulte [`docs/series-data-safety.md`](docs/series-data-safety.md) e [`docs/series-voice-recovery.md`](docs/series-voice-recovery.md).
...
O instalador desta correção é `artifacts/OuroPro6.4-series-cache-first-debug.apk`. Ele mantém o pacote funcional do aplicativo atual, `com.ouropro.player`, com o sufixo de debug `com.ouropro.player.debug`, usa `versionName=6.4` para compatibilidade com a tela de configurações, abre a Home a partir do cache local e sincroniza séries em segundo plano quando o catálogo tem menos de 100 itens. As versões anteriores não devem ser usadas. A assinatura é de debug e serve para teste, não para distribuição comercial.
